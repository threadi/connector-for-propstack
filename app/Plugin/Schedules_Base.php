...
class Schedules_Base {
	...
	public function install(): void {
		...
		if ( wp_next_scheduled( $this->get_name(), $this->get_args() ) ) {
			...
			Log::get_instance()->add( sprintf( __( 'Interval of schedule %1$s changed from %2$s to %3$s - rescheduling.', 'connector-for-propstack' ), $this->get_name(), (string) $current_interval, $interval ), 'info', $this->get_log_category() );
		}
		...
		if ( is_wp_error( $result ) ) { // @phpstan-ignore function.impossibleType
			...
			Log::get_instance()->add( sprintf( __( 'Error during creation of schedule %1$s:', 'connector-for-propstack' ), $this->get_name() ) . ' <code>' . wp_json_encode( wp_json_encode( $result->get_error_messages() ) ) . '</code>', 'info', $this->get_log_category() );
		}
	}
	...
	protected function get_scheduled_interval(): string {
		...
		if ( isset( $this->default_interval, $schedules[ $this->default_interval ] ) && '' !== $this->default_interval ) {
			...
			Log::get_instance()->add( sprintf( __( 'The configured interval %1$s for schedule %2$s is not registered - falling back to %3$s.', 'connector-for-propstack' ), $interval, $this->get_name(), $this->default_interval ), 'error', $this->get_log_category() );
