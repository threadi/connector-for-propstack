...
class Db {
	...
	public function get_results( ?string $sql, string $data_type = ARRAY_A ): array {
		...
		// get the results.
		$results = $wpdb->get_results( $sql, $data_type ); // phpcs:ignore PluginCheck.Security.DirectDB.UnescapedDBParameter,WordPress.DB.PreparedSQL.NotPrepared -- $sql is always a prepared statement supplied by the caller (see method contract).
	 ...
	 * Log any error.
	 *
	 * @param string $sql Used SQL-statement.
	 *
	 * @return void
	 */
	private function log_error( string $sql ): void {
		...
		$text  = '<strong>' . __( 'Database-error occurred!', 'connector-for-propstack' ) . '</strong>';
		$text .= '<br><br><em>' . __( 'Statement:', 'connector-for-propstack' ) . '</em> <code>' . $sql . '</code>';
		$text .= '<br><br><em>' . __( 'Error:', 'connector-for-propstack' ) . '</em> <code>' . $wpdb->last_error . '</code>';
