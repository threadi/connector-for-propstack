...
class SiteOrigin extends Compatibilities_Base {
	 ...
	 * @var string
	 */
	protected string $name = 'cfprop_compatibility_siteorigin';
	...
	public function check(): void {
		...
		if ( $this->is_active() ) {
			...
			$transient_obj->set_message( sprintf( __( 'We realized that you are using Site Origin - very nice! <a href="%1$s" target="_blank"><i>Connector for Propstack Pro</i> (opens a new window)</a> allows you to design the output of positions in Site Origin.', 'connector-for-propstack' ), esc_url( Helper::get_pro_url() ) ) );
