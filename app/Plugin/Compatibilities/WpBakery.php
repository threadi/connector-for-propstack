...
class WpBakery extends Compatibilities_Base {
	 ...
	 * @var string
	 */
	protected string $name = 'cfprop_compatibility_wpbakery';
	...
	public function check(): void {
		...
		if ( $this->is_active() ) {
			...
			$transient_obj->set_message( sprintf( __( '<strong>We realized that you are using WPBakery - very nice!</strong> <a href="%1$s" target="_blank"><i>Connector for Propstack Pro</i> (opens a new window)</a> allows you to design the output of your objects in WPBakery.', 'connector-for-propstack' ), esc_url( Helper::get_pro_url() ) ) );
