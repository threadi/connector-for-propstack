...
/**
 * Object to handle all the Gutenberg templates of this plugin.
 */
class Templates {
	/**
	...
	public function add_block_templates( array $template_list, array $query, string $template_type ): array {
		...
		foreach ( $templates as $template ) {
			...
			// hide the template if the post-types do not match.
			if ( $post_type &&
		...
		// return the resulting list of templates.
		return $template_list;
	}

	/**
	 * Return the supported block templates from the filesystem (plugin-source) AND database (custom templates from user).
	 *
	...
	private function get_block_templates( array $slugs, string $template_type ): array {
		...
		foreach ( $this->get_templates() as $template_slug => $settings ) {
			// ignore the template if it does not match a requested slug (if given).
			...
			$template_obj->set_is_single( str_starts_with( $template_slug, 'single-' ) );
			if ( $template_obj->is_valid() ) {
				$templates[ $template_slug ] = $template_obj;
			}
		}

		// return the merged list of templates from the filesystem AND database.
		return array_merge( $templates, $this->get_templates_from_db( $slugs, $template_type ) );
	}

	/**
	 * Return the block template as an object for the save-request.
	 *
	...
	public function get_block_file_template( null|WP_Block_Template $template, string $id, string $template_type ): null|WP_Block_Template {
		...
		if ( $this->get_parent_id() !== $template_id ) {
			return $template;
		}

		// get the list of our own block templates.
	...
	/**
	 * Return the list of available block templates.
	 *
	...
	private function get_templates(): array {
		// define the list.
		$templates = array(
			'single-' . ImmoObject::get_instance()->get_name()  => array(
				'title'       => __( 'Single object', 'connector-for-propstack' ),
				'description' => __( 'Displays a single object from Propstack.', 'connector-for-propstack' ),
				'source'      => 'plugin',
			),
			'archive-' . ImmoObject::get_instance()->get_name() => array(
				'title'       => __( 'List of objects', 'connector-for-propstack' ),
				'description' => __( 'Displays a list of objects from Propstack.', 'connector-for-propstack' ),
				'source'      => 'plugin',
			),
		);
		 ...
		 */
		return apply_filters( 'cfprop_block_templates', $templates );
	}

	/**
	 * Get templates from the database to override the template from files.
	 *
	...
	public function get_templates_from_db( array $slugs, string $template_type ): array {
		...
		$query = array(
			...
			'tax_query'      => array(
				array(
					'taxonomy' => 'wp_theme',
					'field'    => 'name',
					'terms'    => array( $this->get_parent_id(), get_stylesheet() ),
				),
			),
		);
		...
		foreach ( $check_query->get_posts() as $post ) {
			...
			// create the template object and add its settings.
			$template_obj = new Template();
	...
	/**
	 * Update the db-based templates if the theme has been switched to another block-theme.
	 *
	 * E.g., necessary to adjust the header- and footer-templates.
	 *
	 * @return void
	 * @noinspection PhpUnused
	 */
	...
	/**
	 * Remove our templates from the database.
	 *
	 * @return void
	 * @noinspection PhpUnused
	 */
	...
	/**
	 * Return the Gutenberg-template-parent-ID, which should reflect the plugin-directory.
	 *
	 * @return string
	 */
	public function get_parent_id(): string {
		return 'connector-for-propstack/connector-for-propstack';
	}
}
