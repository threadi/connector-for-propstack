	...
	public function run(): void {
		...
		try {
			...
			if ( $is_first_run ) {
				...
				foreach ( $languages as $language_code => $language_enabled ) {
					...
					if ( get_option( 'cfprop_md5_' . $language_code ) === $md5 && 1 !== absint( get_option( 'propstack_connector_debug', 0 ) ) ) {
						// add a log entry.
						/* translators: a title will replace %1$s. */
						Log::get_instance()->add( sprintf( __( 'The objects in the %1$s language have not been modified. The import will not be performed.', 'connector-for-propstack' ), ' <em>' . $language_code . '</em>' ), 'success', 'import' );
					...
					// add every object of this language to the flat work list.
				...
				// free the memory, the objects are in the work list now.
				unset( $objects, $data );

				// split the work list into blocks so a single run never has to load all objects.
				$block_size = $limit > 0 ? $limit : max( 1, count( $import_data['objects'] ) );
				$blocks     = array_chunk( $import_data['objects'], $block_size );

				// save every block in its own option.
				foreach ( $blocks as $block_index => $block ) {
					update_option( $this->work_list_option . '_block_' . $block_index, $block, false );
				}

				// keep only the small metadata in the main option.
				$import_data['total']      = count( $import_data['objects'] );
				$import_data['blocks']     = count( $blocks );
				$import_data['block_size'] = $block_size;
				$import_data['objects']    = array();

				// free the memory of the blocks.
				unset( $blocks );
				...
				try {
					...
					Log::get_instance()->add(
						...
						/* translators: %1$s will be replaced by the object title, %2$d by its Propstack-ID. */
							...
							'<em>' . esc_html( $object['title']['value'] ?? '' ) . '</em>',
							absint( $object['id'] ?? 0 )
			...
			if ( Helper::is_development_mode() ) {
				Log::get_instance()->add(
					sprintf(
						'Memory after chunk: %1$s of %2$s, work list entries: %3$d',
						size_format( memory_get_peak_usage( true ) ),
						ini_get( 'memory_limit' ),
						count( $import_data['objects'] )
					),
					'info',
					'import'
				);
			...
			if ( $this->has_load_more() ) {
				// this block is done, it is not needed anymore.
			...
			// log this event.
			Log::get_instance()->add(
				__( 'Following error occurred during the import of objects via API v1:', 'connector-for-propstack' )
				. '<br>' . Helper::get_throwable_as_log_text( $e ),
				'error',
				'import'
			);
