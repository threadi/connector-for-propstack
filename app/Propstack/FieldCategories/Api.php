<?php
/**
 * File to handle the API category for fields.
 *
 * @package connector-for-propstack
 */

namespace ConnectorForPropstack\Propstack\FieldCategories;

// prevent direct access.
defined( 'ABSPATH' ) || exit;

use ConnectorForPropstack\Plugin\Settings;
use ConnectorForPropstack\Propstack\Field_Category_Base;

/**
 * Object to handle the API category for fields.
 */
class Api extends Field_Category_Base {
	/**
	 * The internal name of the category.
	 *
	 * @var string
	 */
	protected string $name = 'api';

	/**
	 * Return the category label.
	 *
	 * @return string
	 */
	public function get_label(): string {
		return __( 'Propstack API', 'connector-for-propstack' );
	}

	/**
	 * Return whether the metabox should be hidden (true) or not (false).
	 *
	 * @return bool
	 */
	public function hide_meta_box(): bool {
		return ! current_user_can( Settings::get_instance()->get_settings_obj()->get_capability() );
	}
}
